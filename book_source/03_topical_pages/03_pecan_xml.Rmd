...
... will be translated (by way of
[`PEcAn.DB::db.open`](https://pecanproject.github.io/package-documentation/develop/PEcAn.DB/reference/db.open.html)
(`settings$database$bety`)) into R code ...
...
- [`PEcAn.DB::db.open`](https://pecanproject.github.io/package-documentation/develop/PEcAn.DB/reference/db.open.html)
  —-Open a database connection and create a connection object
,   which is used by many other functions for communicating with the PEcAn database.
...
* `constants`: (optional) this section contains information that will be written directly into the model specific configuration files. For example, some models like ED2 use PFT numbers instead of names for PFTs, and those numbers can be specified here. See documentation for model-specific code for details.
* `posterior.files` (Required unless database is available) this tag helps to signal write.config functions to use specific posterior/prior files (such as HPDA or MA analysis) for generating samples without needing to access to the bety database.

This information is currently used by the following PEcAn workflow function:

- - [`PEcAn.workflow::get.traits`](https://pecanproject.github.io/package-documentation/develop/PEcAn.workflow/reference/get.traits.html)
  — Retrieves trait data for selected PFTs from the database or posterior files
...
Conversely, if you do not want to do a trait meta-analysis (e.g. if you want to manually set all parameters), you should omit this section.
...
- - [`PEcAn.MA::run.meta.analysis`](https://pecanproject.github.io/package-documentation/develop/PEcAn.MA/reference/run.meta.analysis.html)
  — Performs Bayesian meta-analysis of trait data to generate posterior distributions
...
-PEcAn supports a wide range of meteorological driver datasets through the PEcAn.data.atmosphere module.
This documentation provides examples of commonly used meteorological inputs, but the full and up-to-date set of supported drivers is defined in the download and met2CF functions within the data.atmosphere module,

Users are encouraged to consult the source code for the authoritative list of supported datasets:
