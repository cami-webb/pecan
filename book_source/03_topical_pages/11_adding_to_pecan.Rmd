...
2. Integrate the interface into the rest of the PEcAn system
3. Updating the PEcAn database to register the model (optional)
...
#### Other input converters (optional)

Many models accept additional inputs beyond meteorology. Examples include initial site conditions, environmental controls such as soil physics  or hydrology factors or phenology, specifications of management or disturbance events, etc. We encourage model couplers to support these where appropriate, typically via (1) a custom function that handles conversion from PEcAn's input format to model-specific files, followed by (2) code in `write.config.MODEL` that integrates input values into run-specific setup. If write.config.MODEL knows how to work with the format PEcAn provides standard for that input, step (1) can be skipped.

Because not all models accept these inputs and the implementation approach will differ substantially between models, the `pecan.MODEL` package template does not include function skeletons for conversion of non-met inputs. See the code of existing model coupler packages to find examples of how other models convert the PEcAn format for a given input stream into the inputs of interest to a specific model. For example PEcAn.SIPNET uses `write.events.SIPNET` to convert JSON records of agronomic management events into the ASCII format expected by the model, while phenology is read directly from CSV in `write.config.SIPNET` and therefore has no separate conversion function.

See  also [Adding a new input converter](#InputConversions) for more about PEcAn's approach to input handling and what information is passed in each input type.
...
### Integrate MODEL into the PEcAn build system
...
### Add model info to PEcAn Database

Note: As support for running PEcAn with no database expands, this step has become less important, but keep reading -- this section is still the best summary of what information needs to be _available to_ the model, whether looked up from the database on the fly or passed in by hand.
...
![](03_topical_pages/11_images/bety_new_model.png)
...
Current Meteorological products that are coupled to PEcAn can be found in our [Available Meteorological Drivers](#met-drivers) page.
...
knitr::include_graphics("02_demos_tutorials_workflows/02_user_demos/07_advanced_user_guide/images/data-ingest/D1Ingest-1.gif")
```
<br>
...
```{r, echo=FALSE,out.height= "50%", out.width="50%", fig.align='center'}
knitr::include_graphics("02_demos_tutorials_workflows/02_user_demos/07_advanced_user_guide/images/data-ingest/Local_loader_sm.gif")
```
<br>
```{r, echo=FALSE,out.height= "50%", out.width="50%", fig.align='center'}
knitr::include_graphics("02_demos_tutorials_workflows/02_user_demos/07_advanced_user_guide/images/data-ingest/local_browse.gif")
```
...
<br> 
  ...
  <br>
```{r, echo=FALSE,out.height= "50%", out.width="50%", fig.align='center'}
knitr::include_graphics("02_demos_tutorials_workflows/02_user_demos/07_advanced_user_guide/images/data-ingest/Selectize_Input_sm.gif")
```
<br>
...
<br>
```{r, echo=FALSE,out.height= "50%", out.width="50%", fig.align='center'}
knitr::include_graphics("02_demos_tutorials_workflows/02_user_demos/07_advanced_user_guide/images/data-ingest/DateTime.gif")
```
...
<br>
```{r, echo=FALSE,out.height= "50%", out.width="50%", fig.align='center'}
knitr::include_graphics("02_demos_tutorials_workflows/02_user_demos/07_advanced_user_guide/images/data-ingest/new_format_record.gif")
```
...
<br>
```{r, echo=FALSE,out.height= "50%", out.width="50%", fig.align='center'}
knitr::include_graphics("02_demos_tutorials_workflows/02_user_demos/07_advanced_user_guide/images/data-ingest/D1Ingest-9_sm.gif")
```
...
![example_data](02_demos_tutorials_workflows/02_user_demos/07_advanced_user_guide/images/example_data.png)
...
![format_record_1](02_demos_tutorials_workflows/02_user_demos/07_advanced_user_guide/images/format_record_1.png)
...
![format_record_2](02_demos_tutorials_workflows/02_user_demos/07_advanced_user_guide/images/format_record_2.png)
...
![var_record](03_topical_pages/11_images/var_record.png)
