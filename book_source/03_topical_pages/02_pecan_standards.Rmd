...
PEcAn relies on shared data standards to ensure interoperability between
models, workflows, and analysis tools. These standards define how input
data, model outputs, and metadata are structured so that components can
work together consistently across sites and experiments.

This chapter introduces PEcAn’s data standardization philosophy and
provides links to detailed specifications for each standard.

## Contents
- [Input data standards](#input-data-standards)
- [Output data standards](#output-data-standards)
...
Meteorological (met) drivers provide climate forcing data required by
ecosystem models within PEcAn. These datasets vary in spatial resolution,
temporal coverage, and available variables.
...
## Example meteorological drivers
- [Daymet](#daymet)
- [NARR](#narr)
- [ERA5](#era5)
- [PRISM](#prism)
...
drivers. The full and up-to-date list of supported datasets is implemented
in the `download` and `met2CF` functions in the `data.atmosphere` module.
