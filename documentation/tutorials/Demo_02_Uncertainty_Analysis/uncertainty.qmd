...
# 11. Visualize Uncertainty Analysis Results
...
```{r visualize-uncertainty-results}
# Load the ensemble time series data and generate the plot
ensemble_ts_file <- file.path(settings$outdir, "ensemble.ts.Rdata")
if (file.exists(ensemble_ts_file)) {
  load(ensemble_ts_file)
  PEcAn.uncertainty::plot.ensemble.ts(ensemble.ts)
}

# Load the sensitivity analysis data and generate the plots
sa_file <- file.path(settings$outdir, "sensitivity.analysis.Rdata")
if (file.exists(sa_file)) {
  load(sa_file)
  PEcAn.uncertainty::plot.sensitivity.analysis(sa.samples)
}

# Load the variance decomposition data and generate the plot
vd_file <- file.path(settings$outdir, "variance.decomposition.Rdata")
if (file.exists(vd_file)) {
  load(vd_file)
  PEcAn.uncertainty::plot.variance.decomposition(variance.decomposition.output)
}
```
